---
sidebar_label: 'Userid mapping'
title: Mapping z/OS users to OpCon user and token definitions
description: "How to map z/OS userids to OpCon event users and tokens through RACF custom fields, including XPSUSER and XPSTOKEN definitions."
tags:
  - Procedural
...
A mechanism for mapping z/OS userids to OpCon event users and tokens through RACF custom fields. Starting in z/OS Agent 21.00.02, the security administrator can store an OpCon event user and password (token) in the security database for each z/OS user. When those values are defined, the agent uses them when sending external events from z/OS.
...
Beginning with release 21.00.02, the agent add the ability for the security administrator to store an OpCon event user and password (token) in the security database for each z/OS user.  If these values are defined, they will be used on the event.
...
When a user program or trigger sends an event, the agent will extract the custom fields for the current user. If XPSUSER or XPSTOKEN fields are set, their values will be used in place of the z/OS userid and the common password.  If the OpCon userid is the same as the z/OS userid, the XPSUSER field can be omitted.
...
## Allowing users to view or update their own field values
...
When using multiple agent instances, the userid mapping will use XPxUSER and XPxTOKEN for each instance, where ‘x’ is the XPSID.  These fields are defined in the same way as the defaults. 
 
In other words, when an event is being sent by USERA from the agent with XPSID=x, the user will be the first of these to be defined
...
With multiple agent instances, you may want to use generic FIELD profiles to allow user access
