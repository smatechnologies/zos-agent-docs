...
title: z/OS Agent customization
description: "Configure the z/OS Agent's system, security, JES, and run-time parameters required before starting and testing the agent."
tags:
  - Configure
...
The z/OS Agent customization process configures the system, security, JES, and agent run-time parameters that must be in place before you start and test the z/OS Agent.

Before continuing, complete the following:

- Review the agent JCL procedure and the XPSPRMxx run-time parameters.
- Define security profiles that allow the agent to submit batch jobs and use TCP/IP.
- Configure SMF recording for dataset and job tracking exits.
- Configure the TSO environment so authorized commands and ISPF libraries are available.
- If applicable, plan a Sysplex configuration that uses a primary LSAM and one or more PSAMs.

## Agent options and JCL procedures

Before continuing on, review the agent JCL procedure in your system PROCLIB and the XPSPRMxx definition in INSTLIB. The following is a typical agent JCL procedure.
...
Next, you should review your installation agent run-time options in XPSPRM00.

The following is an example of a typical agent Parmlib XPSPRMxx member implementation:
...
|--- |--- |--- |
|ADOPT|*Blank*|Determines if the agent can become the primary LSAM authority during failover.<ul><li>Valid values are Y (yes) and N (no) or a system name.</li><li>The named system will be eligible to become the primary LSAM.</li></ul>|
...
|INTV|00.00.10|This value determines the maximum delay for external resource recognition (batch triggers, Pre-run Data Triggers, and so forth).<br /><br/>A valid value must use the following syntax: `hh.mm.ss`.<br/><br/>The INTV line is supplied commented out in the sample XPSPRMxx member, so a new installation runs at the default of 00.00.10 until that line is uncommented and changed. The parameter displays on this page and under [Verifying the installation](installation/verifying.md) show configured systems, not the shipped default.|
|JCLDD|XPSJCL|Sets the default DDNAME in the agent JCL Proc. It is used to locate JCL for any batch Job Start request from the SAM that does NOT have a DDNAME assigned.|
|LSAM|N|In a Sysplex environment, determines which agent is running as the primary LSAM.<ul><li>This parameter has no affect if SYSPLEX=N is coded. For information on the SYSPLEX parameter, refer to SYSPLEX.</li><li>Valid values are Y (yes), N (no) or a system name.</li></ul>|
...
|MACHINEID|*|Defines the name of the agent.<ul><li>If set to an asterisk (*), the agent uses the system name from SYSNAME.</li></ul>The name may be up to 8 characters long and must match the name entered for the Machine in the SMA Opcon Enterprise Manager.|
...
|MLWTO|YES|Determines if the agent should process minor lines of multi-line console messages.<br/>Valid values are YES and NO.|
|Override-DD|*Blank*|The name of a DD statement in the agent JCL to use as the default override DD, if one is not specified in the job definition.<br/>A blank value means that there will be no default.|
|XPSDYNAM|IEESYSAS|Identifies the cataloged procedure to use for dynamic tasks (REXX and Operator Command events).<br/><br/>The value also forms the procedure portion of the STARTED-class key SAF uses to assign a USERID to each dynamic address space. See [STARTED class — dynamic event address space USERID](reference/saf-resources.md#started-class-dynamic-event-userid).|
...
|PROCESS|030|A number defining the maximum number of jobs the agent can simultaneously manage (1 – 512).<br/>This value is used during the agent's first initialization after an IPL to determine CSA usage. Refer to CSA Storage Allocation on CSA Storage Allocation to determine the impact of this parameter.After initialization, the maximum process count allowed will be limited by the initial allocation.<br/>PROCESS=0 can be used to prevent jobs from starting, similar to setting the machine to LIMITED.<br/>PROCESS=RESET will cause the agent to reset the process count to the value it had before PROCESS=0 was used.|
|RECOVERY|PROMPT|Determines the agent's behavior during recovery from a system failure.<br/>Upon reactivation, the agent searches the RECLOG for jobs that were running at the time the system went down.<br/>If any are found, the operator may be prompted or the agent automatically sends termination messages to the SMANetCom.<br/>If prompted, the operator may select to ignore executing job terminations (for instance, if they were already terminated by the Scheduler and restarts are waiting).<br/>Valid values are PROMPT and AUTO.|
|RESTORE-COND-CODES|YES|Determines whether captured condition codes are restored to the steps that a restart skips.<br/><br/>The agent captures the condition code and abend code of every step on every run. That capture is unconditional and is not controlled by this parameter. This parameter determines only what is done with the captured values when the job is restarted.<br/><br/>If YES, the agent restores the captured codes to the skipped steps, so that backward references in `COND=` and `IF/THEN/ELSE` logic evaluate as though the full job had run.<br/><br/>Setting this parameter to "NO" will leave the skipped steps in "not run" status.|
|SETQUES|Automatic calculations using PROCESS=|Overrides default queue limits. Refer to CSA Storage Allocation prior to setting these values.<br/>Extended CSA (ECSA) is consumed in proportion to these values.<br/>Valid queues include: (JOB=nnn [;MSG=nnn] [;DSN=nnn] [;WTO=nnn] [;EVT=nnn]) <br/><br/>Note: These must be defined in the XPSPARMxx since they are not dynamic. Using the MODIFY command to change these values has no effect on the agent.|
|SPINOFF|Y|Determines if the agent automatically spins off a new LSAM LOG dataset each day at midnight.<br/>If set to N, a SPINLOG command (F lsamname,SPINLOG) should be scheduled each day to ensure the log does not exceed allocated space.<br/>If set to Y, the agent automatically manages the log.<br/>Valid values are Y (yes) and N (no).|
|SYSPLEX|N|Determines if the agent is running in a SYSPLEX environment.<br/>If Y, the XPSPLEX task is automatically started when the agent is started.<br/>Valid values are Y (yes) and N (no).|
...
|SPFAUDIT|N|Determines if the agent issues XPS083I messages for ISPF Users that update or delete DSN Table entries.<br/>Valid values are Y (yes) and N (no).|
...
|AUTOSTEP|YES|Automatic restart step selection. As the job runs, the agent evaluates each completed step and records the nearest step from which the job can safely be restarted, so that a restart does not require an operator to work out the restart point. The recorded step is reset to null when the job ends normally.<br/>Valid values include:<ul><li>YES: Select and record the nearest recoverable restart step as the job runs - reset to null if good EOJ.</li><li>NO: Do not select a restart step automatically. The restart step must be supplied with the restart request.</li></ul>|
|XPRLIST|*Blank*|Two-character suffix for the XPRLSTxx member to load as the dataset cleanup filter table. Refer to [Dataset Cleanup Filter Table](advanced-features/xprlist.md) for filter entry syntax, wildcard patterns, and operator commands.|
...
The agent will attempt to map the full OpCon userid to a z/OS userid through distributed identity mapping, with registry name "OPCON", using a standard SAF call. If the identity mapping fails, then the z/OS userid will be obtained using the previous rules.
...
In the following figure, we use standard TOP SECRET commands to define the necessary permissions to OPCON datasets, and then add *lsamname* to the Started Task Class. We then define the *lsamname* agent as a *surrogate* submitter for Batch Jobs. We then give *lsamname* an OMVS segment descriptor allowing native IP communication.
...
You must provide the following SMF environment for the z/OS Agent.
...
1. **Scheduling Sysplex LSAMs or JES MAS from a master system.** This option requires a Sysplex configuration. Only one machine that participates in scheduling is defined to SAM ("Machine ID") and the LSAM on that system manages the routing of all JCL, regardless of the Sysplex member on which it eventually runs. This is the recommended option for Clients utilizing a single Sysplex or JES2 Multi-Access Spool (MAS) configuration. In this configuration, one system is chosen as the master (Initiates an "LSAM" Gateway) and all others in the participating Sysplex are partition/Plex clients (Initiates a "PSAM" Monitor) whether or not they share spool or DASD volumes. The primary LSAM is defined in the SAM "Machine ID" option.
2. **Scheduling independent machines.** This option requires that every machine that participates in scheduling be defined to SAM and the agent on each system submits all JCL for that system. This is the recommended option for Clients utilizing separate JES2 environments. In this configuration, every system is considered a master (Initiates an "LSAM") and the system on which JCL submission and execution occurs is defined in the SAM "Machine ID" option.

Both the above options can be used in the same Data Center. This is especially helpful in outsourcing or consolidated service environments. Dependencies among and between LSAMs and PSAMs in the data center aresupported by SMA Opcon SAM engine. Whether or not the agent main task runs as an LSAM or a Sysplex PSAM is controlled by runtime parameters on z/OS (LSAM=Y/N and SYSPLEX=Y/N). The default is to run as an LSAM master with Sysplex support turned off (SYSPLEX=N).
...
As illustrated in the figure under [Scheduling a SYSPLEX](#scheduling-a-sysplex), you may "mix-and-match" Gateways and independent LSAMs in the SAM configuration, as well as JES2 and JES3 systems. However, if all the LSAMs are members of the same Sysplex, then XPS390 can use the Sysplex message facility to route job submission from a single gateway, even across two or more JES2 MAS environments. In that case, all LSAMs, except the master gateway(s) must be running a PSAM environment.
